...
export default function AdminLoginPage() {
  ...
  async function signIn(e: FormEvent) { e.preventDefault(); setLoading(true); const { error } = await supabase.auth.signInWithPassword({ email, password }); setLoading(false); if (error) { alert('פרטי התחברות שגויים'); return; } router.push('/admin'); router.refresh(); }
